...
![5 State Model. From @process_states_1]
...
![Simplified Priority Levels. Source @priority_levels_1]
...
With this primitive, some thread can control the execution flow of another thread, by setting or clearing the Event. Can be usefull when you have a thread that is waiting for some other thread to finished something, like if one is going to read from a file but the other has not finished writing to it.
...
In this eval we will compare execution times of a dumb CPU-bound task (iteration a million times) when doing all the work in one Thread versus splitting the work in multiple Threads.
...
With this information, we can calculate the pearson correlation between the time taken for each number of threads and the number of context switches caused by each number of threads.
...
Sadly, measurement of context switches per thread is not possible with the `psutil` library. Although in Unix based systems would be quite straightforward to do a small custom code to get those numbers, tanks to the `/proc` filesystem, which diligently puts all the info we need in a single file per thread that we can open and read. In Windows, though, the necessary steps are **waaaaay** more complicated and the result not nearly as good as in Unix systems.
...
Altough, i will warn now that i will not reproduce the prior results with these new measurement methods, since it would be a lot of extra platform-dependant code and not very useful for the main point of this section, which has already been established.
...
python_threads = [re.findall(pattern, s)[0] for s in win32pdh.ExpandCounterPath('\\Thread(*)\\ID Thread') if 'python' in s]
...
This was the final piece of the puzzle we needed. Calculate the context switches per second of our thread has become possible with the use of the following function:
